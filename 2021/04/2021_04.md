...
Have two classes Line and Desk. Iterate over numbers and get a first board that wins, use the break to stop loops.

## Second task.

The same as previous but remove `breaks` to stop on a first win and add `next` to skip decks that already won (has fully selected line).
