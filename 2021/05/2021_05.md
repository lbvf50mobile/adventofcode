...
For example `arr.map!(&:strip!)` after `arr.clone` changes the appearance of both arrays. Because `arr.clone` creates a new array but with the same strings objects. And a `strip!` method changes the same strings objects for both arrays. This situation could be explained as two different arrays with the pointers to the same objects.
